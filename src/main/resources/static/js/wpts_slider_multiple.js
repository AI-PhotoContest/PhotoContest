...
!function ($) {
  ...
  * ========================= */
  ...
  , slide: function (type, next) {
      ...
      $next = $next.length ? $next : this.$element.find('.item-banner')[fallback]()
 	...
 	/* TSSLIDER CLASS DEFINITION
 	 ...
 	 WptsSlider.prototype = {
		...
		refineslider:function(main_id,element,effects,css_var,autoplay){
			...
			jQuery(element+' .item-banner').each(function(index, element) {
			...
			//'carousel-custom'
		 get_height=jQuery(element+' .item-banner').height();
		 jQuery(element).refineSlide({
			...
			jQuery(element+' .rs-slider li').addClass('item-banner');			
		},
